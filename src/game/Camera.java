...
public class Camera extends Rectangle
{
	...
	{	
		...
		drawObjects(gameObjects, scenarioDimension, g);
		
		player.draw(playerDrawPosX, playerDrawPosY, g);
		player.draw(player.getX(), player.getY(), g);
	}
	
	private void drawObjects(ArrayList<ArrayList<GameObject>> gameObjects,
			Dimension scenarioDimension, Graphics g)
	{	
		for (ArrayList<GameObject> arrayListOfObjects : gameObjects)
		{
			for (GameObject currentObject : arrayListOfObjects)
			{	
				double diffX = this.getX() - displayRect.getX();
				double diffY = this.getY() - displayRect.getY();
	...
	private void calculatePlayerDrawPos(Player player, Dimension scenarioDimension)
	{	
